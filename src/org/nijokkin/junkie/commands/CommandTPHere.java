...
public class CommandTPHere implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		...
		{
			// Teleport target to sender command start.
			Player target;
			Player source;
			Location targetLocation;
			
			// Get the source's object
			...
			if(!(sender instanceof Player))
			{
				...
				// Get the player from the target
				target = (Player) sender;
			}
			
			// Check sender has permission
			if(!CommandUtil.playerPermission(sender, "tp.here"))
			{
			...
			// Notify players
			source.sendMessage(Junkie.COLOR + "You have been teleported to " + target.getDisplayName() + "!");
			target.sendMessage(Junkie.COLOR + source.getDisplayName() + " has been teleported to you!");
		}
